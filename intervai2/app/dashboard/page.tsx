...
export default function DashboardPage() {
  ...
  const t = {
    ...
    quickCard: isDark ? "bg-[#141b2d] border-white/5 card-hover" : "bg-white border-gray-200 shadow-sm card-hover",
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <button onClick={() => router.push("/interview")} className="shimmer-btn flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-transform hover:scale-[1.03] shadow-lg shadow-blue-600/30 whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
        ...
        ].map(({ label, icon: Icon, iconBg, iconClr, val, unit, extra }, i) => (
          <div key={label} className={`${t.card} border rounded-2xl p-5`} style={{ animation: `fadeUp 0.4s ease ${i * 80}ms both` }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <Link href="/dashboard/streaks" className={`group ${t.quickCard} border ${t.quickHoverOrange} rounded-2xl p-5 flex items-center gap-4 transition-all duration-200`} style={{ animation: "fadeUp 0.4s ease 200ms both" }}>
        ...
        <Link href="/dashboard/analytics" className={`group ${t.quickCard} border ${t.quickHoverBlue} rounded-2xl p-5 flex items-center gap-4 transition-all duration-200`} style={{ animation: "fadeUp 0.4s ease 280ms both" }}>
      ...
      <div className={`${t.card} border rounded-2xl p-6`} style={{ animation: "fadeUp 0.4s ease 360ms both" }}>
        ...
        <div className="space-y-2">
          {filtered.map((s, i) => (
            <div key={s.id} className={`grid grid-cols-[1fr_100px_110px_90px_90px] gap-4 items-center px-3 py-3.5 rounded-xl border border-transparent chip-hover cursor-default ${t.rowHover}`} style={{ animation: `slideDown 0.3s ease ${i * 50}ms both` }}>
